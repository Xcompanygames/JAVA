...
public class FilterGui {
	...
	public FilterGui (InputFilterGui filter, ArrayList<Container> file) throws ParseException {
		...
		if (filter.getFilterstatus().equals("L")) {
			...
			radiuscheckfilter();
              ...
              for (int i=0;i<ContainerFiltered.size();i++){
                    if (!check.contains(ContainerFiltered.get(i).getIDList().get(0))){
                        check=check+ContainerFiltered.get(i).getIDList().get(0);
                        devicescount++;
                    }
                }
